...
class StockData_merge_index():
    """
        훈련용 데이터세트와 평가용 데이터세트를 만들기 위한 과정.
        INDEX_DATA를 따로 입력하는 것이 아니라 corr 계산에 같이 들어감!
        [T-window:T-1] => [T+h] 예측
    ...
    def loadCSV(self):
        ...
        #indexdata포함
        self.indexData["Date"] = pd.to_datetime(self.indexData["Date"])
        dataframe = pd.merge(dataframe,self.indexData,how="left",on="Date")#dataframe 기준!
        dataframe.sort_values(by=['Date'],inplace=True)
